class LRUCache {
    ...
    public LRUCache(int capacity) {
        ...
        this.left = new Node(0, 0);  // the left wall 
        this.right = new Node(0, 0); // the right wall 
    ...
    public void put(int key, int value) {
        // if key exist, remove the old node
        if (cache.containsKey(key)) {
            remove(cache.get(key));
        }
        // create new node and put into the map and MRU
        Node newNode = new Node(key, value);
        cache.put(key, newNode);
        insert(newNode);

        // when over limit, remove the LRU node 
